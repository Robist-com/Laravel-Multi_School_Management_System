<strong>Select Criteria</strong>
                    ...
                    <div class="row">
                       <form action="{{route('postfee_statementReport')}}" method="POST">
                        ...
                        <select name="roll_no_feestatement" id="roll_no_balance" class="form-control">
                        <option value="" selected="true">---Select Student---</option>
                        @foreach($students as $key => $student)
                        <option value="{{$student->username}}" @if($student->username === request('roll_no_feestatement')) selected @endif>{{$student->first_name}} {{$student->last_name }}</option>
                        @endforeach
                        </select>
                        </div>
 ...
 #loader{
   visibility:hidden;
            ...
            <div class="row" >
                ...
                <div class="form-group col-sm-3 {{ (request()->segment(1,2) == 'fee statement') ? 'active' : '' }}">
                       <img width="115" height="115" src="{{asset('student_images/' .$detail->image)}}" alt="" srcset="">
                </div>
                <div  class="form-group col-sm-2 {{ (request()->segment(1,2) == 'fee statement') ? 'active' : '' }}">
                      <label class="sidebar-link" >Name : </label>
                </div>

                <div class="form-group col-sm-2 {{ (request()->segment(1,2) == 'balance') ? 'active' : '' }}" >
                    <label class="sidebar-link name"> {{$detail->full_name}}</label>
                </div>

                <div class="form-group col-sm-2 {{ (request()->segment(1,2) == 'fee collection') ? 'active' : '' }}">
            ...
            <table id1="datatable-buttons" class="table table-striped table-bordered">
                <thead >
        ...
        <td style="text-align:center;">@if($data->status == 'paid') <span class="label label-success ">Paid</span> @else <span class="label label-danger">UnPaid</span> @endif</td>
        <td style="text-align:center;">{{$data->class_name}}</td>
        <td style="text-align:center;">{{number_format($data->paid_amount,2)}} <br> <i data-toggle="tooltip" data-placement="right" title="collected by {{$data->name}} ({{$data->invoice_id}})"><i class="fa fa-user fa-lg pull-left"></i><i class="fa fa-mail-forward pull-left"></i></i></td>
