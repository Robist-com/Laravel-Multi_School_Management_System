                        ...
                        <div class="col-md-12">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select id="class" id="class_id" name="class_id"
                                        class="form-control bootstrap-select">
                                        <option value="">Select</option>
                                        @foreach($classes2 as $class)
                                        <option value="{{$class->class_code}}" @if($class->class_code ===
                                            request('class_id')) selected @endif >{{$class->class_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group ">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="form-line">
                                            <input type="text" class="form-control datepicker" name="yearly_date"
                                                required data-date-format="yyyy" value="{{$year}}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <button  class="btn bg-teal btn-round pull-right"
                                        type="submit"><i class="glyphicon glyphicon-print"></i> Get List</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <form action="{{url('/yearly_attendance')}}" method="get">
                    <input type="hidden" class="form-control" name="yearly_date" id="attendance_year" placeholder="Year"
                        value="{{request('yearly_date')}}">
                    <input type="hidden" class="form-control" name="class_id" id="class_id" placeholder="Year"
                        value="{{request('class_id')}}">
                    @if(count($yearly_attend) != 0)
                    <div class="modal-footer">
                        <button type="submit" class="btn bg-red btn-round btn-sm"><i class="fa fa-pdf-o"></i> PDF
                            Report</button>
                    </div>
                    @endif
                </form>
                <b class="col-md-4">Class Wise Report</b>
                <hr>
                <div class="table-responsive">
                <table id="datatable-responsive" class="table table-striped table-bordered js-exportable">
                    <thead>
