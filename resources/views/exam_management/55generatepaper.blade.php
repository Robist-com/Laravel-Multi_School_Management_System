              ...
              <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <div class="input-group">
                      ...
                      @if(isset($subjects))
                      {{ Form::select('course_id',$subjects,$formdata->subject,['class'=>'form-control','id'=>'subject','required'=>'true'])}}
                      @else
                      <select id="subject" id="subject" name="course_id" required="true" class="form-control" required >
      ...
      $.ajax({
        ...
        success: function( json ) {
          ...
          $.each(json, function(i, subject) {
            ...
            $('#subject').append($('<option>').text(subject.course_name).attr('value', subject.id));
          });
        }
      });
    };
...
function getchapter()
{
     ...
     //alert(section);
    $.ajax({
      url: "{{url('/chapter/getList')}}"+'/'+aclass+'?course_id='+subject,
      data: {
        format: 'json'
      },
      error: function(error) {
