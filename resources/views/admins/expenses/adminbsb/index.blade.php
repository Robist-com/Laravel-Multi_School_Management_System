    ...
    <div class="row clearfix">
        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
            <div class="card">
                ...
                <div class="body">
                    @if(isset($expense))
                    {!! Form::model($expense, ['route' => ['expenses.update', $expense->id], 'method' => 'post', 'class' =>
                    'form-horizontal form-label-left', 'enctype' => 'multipart/form-data']) !!}
                    @csrf
                    @else
                    {!! Form::open(['route' => 'expenses.store', 'class' => 'form-horizontal form-label-left', 'enctype'
                    => 'multipart/form-data']) !!}
                    @csrf
                    @endif
                    ...
                    <div class="form-group">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            ...
                            <select name="expense_type_id" id="expense_type_id" class="form-control bootstrap-select"
                                id="bootstrap-select">
                                <option value="0" selected="true" disabled="true" style="margin-right:20px">Select
                                    expense Type</option>
                                @foreach($e_type as $key => $type)
                                <option value="{{$type->id}}" @if(isset($expense))
                                    {{$type->id == $expense->expense_type_id ? 'selected' : ''}} @endif>{{$type->type}}
                                </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="form-line">
                                <input type="text" name="name" id="name" class="form-control" placeholder="Enter Name"
                                    @if(isset($expense)) value="{{$expense->name}}" @endif autocomplete="off">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="form-line">
                                <input type="text" name="invoice_number" id="invoice_number" class="form-control"
                                    placeholder="Enter Invoice Number" @if(isset($expense))
                                    value="{{$expense->invoice_number}}" @endif autocomplete="off">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="form-line">
                                <input type="text" name="date" id="date" class="form-control" placeholder="Enter Name"
                                    @if(isset($expense)) value="{{$expense->date}}" @endif autocomplete="off">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="form-line">
                                <input type="number" name="amount" id="amount" class="form-control"
                                    placeholder="Enter Name" @if(isset($expense)) value="{{$expense->amount}}" @endif
                                    autocomplete="off">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="form-line">
                                <input type="file" name="file_document" id="file" class="form-control"
                                    placeholder="Enter Name" @if(isset($expense)) value="{{$expense->file}}" @endif
                                    autocomplete="off">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <select name="status" id="status" class="form-control bootstrap-select">
                                @if(isset($expense))
                                <option value="1" @if($expense->status == 1) selected @endif> Active </option>
                                <option value="0" @if($expense->status == 0) selected @endif> In Active </option>
                                @else
                    ...
                    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive js-exportable"
                        ...
                        <tbody>
                            ...
                            <tr class="even pointer">
                                ...
                                {!! Form::open(['route' => ['expenses.delete', $expense->id], 'method' =>
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a href="{!! url('print-class-single', [$expense->id]) !!}"
                                                        target="__blank"><i class="glyphicon glyphicon-print"></i>
                                                        Print</a></li>
                                                <li role="separator" class="divider"></li>

                                                <li> <a data-toggle="modal" data-target="#class-view-modal"
                                                        data-batch_id="{{$expense->id}}"
                                                        data-class_name="{{$expense->class_name}}"
                                                        data-class_code="{{$expense->class_code}}"
                                                        data-created_at="{{$expense->created_at}}"
                                                        data-updated_at="{{$expense->updated_at}}"><i
    ...
    <div class="modal fade" id="expense-type" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{route('expenses.store')}}" method="POST" id="frm-level-create"
